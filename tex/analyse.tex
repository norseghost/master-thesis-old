...
\texttt{wordfish}-algoritmen\todo{hvordan formatere WORDFISH?} beregner positioner i én dimension, fortolket som en venstre-højre akse ud fra dokumenternes ordfordeling sig i mellem.
...
\end{figure}\todo{David: dette er ikke de endelige data; men vi kan i det mindste se, hvordan graferne vil arte sig}
er ser vi, hvordan blokkene har ændret position i hver analyseperiode.
Der er nogen bevægelse i de forskellige blokke, men de relative positioner er forholdsvis stabile\todo{skal ændres ift de faktiske data}.
...
Ved at se på figur ~\ref{fig:coef_blocXperiod}\todo{beskriv hvorfor figuren har den facon den har}, fremgår der, at ord som ?????, ??????, ????? indikerer en venstrerettet position; og ord som ???? ???? ?????? ?????? angiver en højreposition.
...
Figur ~\ref{fig:sentposneg} viser partiers\footnote{Jeg holder mig til det udvalg af partierjeg har benyttet mig af tidligere} spredning mellem lave og høje totalværdier for sætninger\todo{analyse på blokniveau?}, fordelt over mine analyseperioder.
