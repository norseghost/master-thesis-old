...
For dette speciales vedkommende, hvor teksten er referater af folketingstaler, vil jeg antage, at afstanden til det spil, der foregår på den politiske arena er forholdsvis kort. \todo{evt komme med eksempler på hvad/hvordan det politiske spil kan kendetegnes?}
...
Datasættet\footnote{Typisk kaldet et \textit{korpus} indenfor tekstbehandlingsverdenen} er dannet ved, at hente mødereferaterne i PDF-form fra Folketingstidende, og er derefter blevet omdannet til tekstformat, og forsynet med metadata i form af fx mødets titel, dato, talerens navm og partiforhold, ved hjælp af computerbiblioteket \citetitle{pedersenFolketinget2019} \autocite{pedersenFolketinget2019}.
...
Jeg programmerer mine bearbejdninger og analyser i R, der er et programmeringssprog specifikt udarbejdet til statistik-analyse.\todo{kilde: hvad er R}.
...
Jeg har derfor en kopi af min kode tilgængeligt på GitHub, til frit brug og offentlig anskuelse \autocite{andersenNorseghostMasterthesis2020}.
...
Også her kan datasettets kompleksitet reduceres, ved at fjerne de termer, der ikke findes i ret mange dokumenter.

Jeg vil igen understrege, at denne forhándsbearbejdning er og vil altid være baseret på forskerskøn.
Det er agtpåliggende for forskeren, at navigere i de mange muligheder.
...
En gennemgående udtalelse indenfor databehandling er, at “data er rodete”\todo{kilde: data is messy}.
For at gøre det ekstra spændende, er de forskellige rå data rodede på hver sin måde.
For indeværende opgave har jeg bemærket:
\begin{itemize}
  \item
  ...
  \item
    dette medfører også, at spredningen af termer over dokumenter er forskøvet ret kraftigt til højre\todo{metodebog: slå op kurveforskydninger}, med mange forholdsvis lave vægtninger — det vil sige; der er mange termer der forekommer i mange dokumenter
  \item
    Datasettet er forsynet med et rigt sæt metadata.\todo{er dette for repetativt ift drøftelse af fejlkilder ovenfor?}
...
\textit{Unigrams} — et ord af gangen — giver et godt billede af frekvenser.\todo{se på ngrams — dette skal omskrives til faktisk praksis}
...
\subsubsection{LDA}
Den algoritme jeg benytter mig af i denne del af processen er \texttt{LDA}\todo{uddyb LDA}\footnote{\texttt{LDA} står for \textit{Latent Dirichlet Allocation}
emner (topics) er en distribution af tokens over alle dokumenter
...
Alle tokens tildeles alle emner, med forskellige.
...
Antallet emner til en LDA-algoritme skal dog specificeres på forhånd.
\footnote{Dette er en såkaldt ikke-superviseret tilgang til tekstanalyse --- andre tilgange baserer sig på, at computerprogrammet ved noget om strukturen af data,m for eksempel ved tilførelse af annotationer inden analyse}.
...
Dette kan biblioteket \texttt{ldatuning} til R hjælpe med, der kan sammenholde forskellige beregninger\todo{find kildehenvisninger}
der \footnote{(((kildehenvisninger til disse metoder)))} til udvælgelse af antal emner i et ellers ukendt korpus.

Dernest udvælger jeg et antal emner der virker fornuftigt på baggrund af denne proces, og udforsker indholdet af de forskellige computergenererede emner\footnote{Til denne eksplorative proces tager jeg biblioteket \texttt{LDAvis} i brug. Dette program udarbejder en interaktiv webside, hvor man kan se, hvilke tokens er fremtrædende i hvilke emner. Den interesserede læser kan se mine generede visualisationer på (((KILDE LDAVIS- læg op på blog ))) }.
Jeg noterer, hvilke emner der eventuelt ser ud til at omhandle uddannelse, og tager disse fra for videre analyse.
...
Jeg gentager nu processen ovenfor med en mindre samling dokumenter, for at se 
